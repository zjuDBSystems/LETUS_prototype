...
public:
    ...
    tuple<uint64_t, uint64_t, uint64_t> WriteValue(uint64_t version, const string& key, const string& value) { // append a new record to the log stream
        ...
        ofstream file("/mnt/d/zju/Research/LETUS_prototype/data/data_file_" + to_string(current_fileID_), ios::app); // create or open the file for appending
        if (!file) {
    ...
    string ReadValue(const tuple<uint64_t, uint64_t, uint64_t>& location) {
        ...
        ifstream file("/mnt/d/zju/Research/LETUS_prototype/data/data_file_" + to_string(fileID)); 
        if (!file) {
