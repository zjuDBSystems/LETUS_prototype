...
int LSVPS::GetNumOfIndexFile() {
    return index_files_.size();
}

void LSVPS::RegisterTrie(DMMTrie *DMM_trie) {
    trie_ = DMM_trie;
}
...
Page* LSVPS::pageLookup(const PageKey &pagekey) {
    ...
    auto file_iterator = std::find_if(
        index_files_.begin(), 
        index_files_.end(),
        [&pagekey](const IndexFile &file) { 
            return file.min_pagekey <= pagekey && pagekey <= file.max_pagekey; 
        }
    );
...
Page* LSVPS::readPageFromIndexFile(const IndexFile& file, const PageKey& pagekey) {
    ...
    auto mapping = std::find_if(
        index_block.GetMappings().begin(), 
        index_block.GetMappings().end(),
...
void LSVPS::applyDelta(BasePage *basepage, const DeltaPage *deltapage, PageKey pagekey) {
...
// MemIndexTable实现
LSVPS::MemIndexTable::MemIndexTable(LSVPS &parent) 
    : parent_LSVPS_(parent) {}
...
void LSVPS::MemIndexTable::Flush() {
    ...
    parent_LSVPS_.AddIndexFile({
        buffer_.front()->GetPageKey(),
        buffer_.back()->GetPageKey(),
        filepath
    });
